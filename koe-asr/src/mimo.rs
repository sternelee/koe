...
fn parse_sse_line(line: &str) -> Option<AsrEvent> {
    ...
    if let Ok(json) = serde_json::from_str::<serde_json::Value>(data) {
        ...
        // Extract text from choices[0].delta.content (streaming) or
        // choices[0].message.content (non-streaming, via extract_text_from_response)
        let text = json
            .get("choices")
            ...
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .or_else(|| extract_text_from_response(&json));
